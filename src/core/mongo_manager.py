...
class MongoManager:
    ...
    def _get_collection_to_operate(self, operation_collection_name: Optional[str], operation_type: str) -> Optional[Collection]:
        ...
        if operation_collection_name:
            ...
            log_collection_name = str(self.collection_name) if self.collection_name else "ativa_sem_nome"
    ...
    def add_data(self,
        ...
        if collection_to_use is None:
            return False # _get_collection_to_operate já logou o erro

        if not data:
            logger.info(f"nenhum dado fornecido para adicionar na coleção '{collection_to_use.name}'.")
            return True
        try:
            result = collection_to_use.insert_many(data, ordered=False)
    ...
    def extract_data(self,
        ...
        if collection_to_use is None:
            return [] # _get_collection_to_operate já logou o erro
    ...
    def delete_all_documents(self, target_collection_name: Optional[str] = None) -> bool:
        ...
        if collection_to_clear is None:
            return False # _get_collection_to_operate já logou o erro
